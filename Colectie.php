...
// =====================================
$ranges = [
    ['type' => 'PsyGeometry', 'min' => 1, 'max' => 60],
    ['type' => 'Ingerasi', 'min' => 61, 'max' => 67],
    ['type' => 'Mandala', 'min' => 71, 'max' => 72],
    ['type' => 'Tablouri', 'min' => 81, 'max' => 93],
    ['type' => 'PsyGeometry Rotunde', 'min' => 131, 'max' => 147]
];

if ($imgNum !== null) {
    foreach ($ranges as $r) {
        if (isset($r['values']) && in_array($imgNum, $r['values'])) {
            return $r['type'];
        }
        if (isset($r['min']) && isset($r['max']) && $imgNum >= $r['min'] && $imgNum <= $r['max']) {
            return $r['type'];
        }
    }
}
...
$typesAnchors = [
    'PsyGeometry' => 'PsyGeometry',
    'Ingerasi' => 'Ingerasi',
    'Mandala' => 'Mandala',
    'Tablouri' => 'Tablouri',
    'PsyGeometry Rotunde' => 'PsyGeometryRotunde'
];
?>
<!DOCTYPE html>
...
<div style="text-align:center;margin:20px 0;" id="filters">
    <button class="filter-btn active" data-filter="PsyGeometry">Covoare dreptunghiulare ale păcii</button>
    ...
    <button class="filter-btn" data-filter="PsyGeometry Rotunde">Covoare rotunde ale păcii</button>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Funcția de aplicare a filtrului
    function applyFilter(btn) {
        filterButtons.forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        const filter = btn.dataset.filter;
    ...
    // Filtrul implicit pe butonul activ
    let activeBtn = document.querySelector('.filter-btn.active') || document.querySelector('.filter-btn');

    // Dacă există hash în URL, îl folosim doar pentru selectarea butonului, fără scroll
    if (window.location.hash) {
        const hash = decodeURIComponent(window.location.hash.substring(1)); // ex: PsyGeometry
        const btnFromHash = Array.from(filterButtons)
            .find(b => b.dataset.filter.replace(/\s+/g,'') === hash.replace(/\s+/g,''));
        if (btnFromHash) {
            activeBtn = btnFromHash;
        }
    }
    ...
    // Click pe butoanele de filtrare
    filterButtons.forEach(btn => {
        btn.addEventListener('click', () => applyFilter(btn));
    });
