...
if ($is_post && ($payment_method === 'online' || ($_POST['payment_method'] ?? '') === 'online')) {
   ...
   \Stripe\Stripe::setApiKey('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');
    ...
    try {
        ...
        // TRIMIT EMAIL (magazin + optional client)
        try { sendOrderEmail($pdo, $order_id, $STORE_EMAIL); } catch (Exception $e) { error_log('sendOrderEmail error (online initial): ' . $e->getMessage()); }
        // CONFIRMARE CLIENT
        try { sendClientConfirmationEmail($pdo, $order_id); } catch (Exception $e) { error_log('sendClientConfirmationEmail error (online initial): ' . $e->getMessage()); }
