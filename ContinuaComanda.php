...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // sanitizare si valori implicite
    ...
    try {
        ...
        /* =========================
           LOGICA PENTRU EASYBOX (FARA INSERT)
           - luam easybox_id din POST
           - fallback la sesiune (daca ai setat acolo din AJAX)
        ========================== */
        if ($shipping_method === 'easybox') {
            // prioritate: POST -> sesiune
            $easybox_id = (int)($_POST['easybox_id'] ?? $_SESSION['selected_easybox_id'] ?? 0);
            if ($easybox_id <= 0) {
                throw new Exception("EasyBox nespecificat");
            }

            $stmt = $pdo->prepare("SELECT id, name, address, city, county, lat, lng, active FROM easyboxes WHERE id = ? LIMIT 1");
            ...
            // construim address-ul complet din coloanele tabelului easyboxes
            $shipping_address = "{$box['county']}, {$box['city']}, {$box['address']}";
        } else {
            /* =========================
               DATE LIVRARE PENTRU CURRIER (adresă user)
            ========================== */
            $address = $addresses[0] ?? null;
            if ($address) {
                $shipping_name    = $address['name'];
                $shipping_phone   = $address['phone'];
                // folosește câmpul address_line existent în tabela user_addresses
                $shipping_address = "{$address['county']}, {$address['city']}, {$address['address_line']}";
            } else {
                // fallback: date minimale (ar trebui rare)
        ...
        $order_total   = round(floatval($total) + $shipping_cost + $payment_fee, 2);
        ...
        foreach ($cart as $item) {
            ...
            // inseram linie comanda
            $stmtItem->execute([$order_id, $product_id, $qty, $price]);

            // actualizam stoc, validam disponibilitate
...
<div id="pickup-modal" class="address-modal" style="display:none;">
  <div class="modal-content">
    ...
    <div style="display:flex; gap:20px;">
      <!-- Stânga: filtre și listă -->
      <div style="flex:1;">
        <div class="pickup-buttons">
          <button type="button" id="filter-city-btn" class="btn-main">Oras</button>
          <button type="button" id="filter-county-btn" class="btn-main">Judet</button>
        </div>

        <!-- Bara de căutare apare doar când apeși pe un buton -->
        <div id="filter-input-container" style="margin-top:10px; display:none;">
          <input type="text" id="filter-input" placeholder="Caută...">
        </div>

        <div id="pickup-list" style="max-height:400px; overflow-y:auto; margin-top:10px;">
          <?php foreach($easyboxes as $box): ?>
            <div class="easybox-item"
              ...
              <p><?= htmlspecialchars($box['city']) ?>, <?= htmlspecialchars($box['county']) ?></p>
            </div>
          <?php endforeach; ?>
        </div>

        <button id="close-pickup-modal" class="pu-btn" style="margin-top:10px;">Închide</button>
      </div>
...
function populateModalListFromPage() {
  ...
  // golim lista de fiecare data (IMPORTANT)
  modalList.innerHTML = '';

  // lista-sursa REALA (de pe pagina)
  const sourceItems = document.querySelectorAll('#pickup-list-page .easybox-item');

  if (!sourceItems.length) {
    console.warn('Nu exista easybox-uri de copiat din pagina');
    return;
  }
  ...
  sourceItems.forEach(el => {
    ...
    // siguranta: verificam coordonate
    if (!clone.dataset.lat || !clone.dataset.lng) {
      console.warn('Easybox fara coordonate:', clone);
      return;
    }
...
function attachActionsToItems() {
  $$('#pickup-list .easybox-item').forEach(item => {
    ...
    actions.innerHTML = `
      <button type="button" class="btn-use">Foloseste acum</button>
      <button type="button" class="btn-save">Salveaza ca punct favorit</button>
    `;
    item.appendChild(actions);

    const btnUse = actions.querySelector('.btn-use');
    ...
    btnUse.addEventListener('click', (ev) => {
      ...
      const pickupInput = document.getElementById('pickup_location');
      if (pickupInput) pickupInput.value = name + ' | ' + address;
      const ship = document.getElementById('shipping_method');
      ...
      const pickupModal = document.getElementById('pickup-modal');
      if (pickupModal) pickupModal.style.display = 'none';
      try { localStorage.setItem('selected_easybox', JSON.stringify({ name, address, lat: parseFloat(lat), lng: parseFloat(lng), city: item.dataset.city, county: item.dataset.county })); } catch(e){}
    });
    ...
    // optional: click pe item actualizeaza preview
    item.addEventListener('click', () => {
      const easyNameEl = document.getElementById('easybox-name');
      const easyAddrEl = document.getElementById('easybox-address');
      if (easyNameEl) easyNameEl.textContent = name;
      if (easyAddrEl) easyAddrEl.textContent = address;
    });
  });
...
// helper: normalizeaza string pentru comparare
function normalizeForSearch(s){
  if(!s) return '';
  return s.toString().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
}

// reverse geocode latlng -> judet (administrative_area_level_1)
function reverseGeocodeCounty(latlng) {
      ...
      for (let r of results) {
        ...
        for (let comp of r.address_components) {
          if (comp.types && comp.types.indexOf('administrative_area_level_1') !== -1) {
            return resolve(comp.long_name || comp.short_name || null);
          }
        }
      }
      resolve(null);
...
// =======================
// CREARE HARTA GENERICA CU FILTRARE AFTER COUNTY
// =======================
function createEasyBoxMap(mapElementId, markersArray, countyFilter = '', listSelector = '#pickup-list .easybox-item') {
  const mapEl = document.getElementById(mapElementId);
  if (!mapEl || !window.google) return null;

  if (Array.isArray(markersArray)) {
    markersArray.forEach(m => { try { m.setMap(null); } catch(e){} });
  ...
  // SCOPE: folosim listSelector in loc de .easybox-item global
  document.querySelectorAll(listSelector).forEach(div => {
    const lat = parseFloat(div.dataset.lat);
    const lng = parseFloat(div.dataset.lng);
    const city = div.dataset.city || '';
    const county = div.dataset.county || '';

    if (!Number.isFinite(lat) || !Number.isFinite(lng)) return;
    if (countyFilter) {
      if (normalizeForSearch(county) !== normalizeForSearch(countyFilter)) return;
    }
    ...
    marker.addListener('click', () => {
      applyBoxToForm(box, { closeModal: true });
  ...
  if (added === 0) {
    try {
      const saved = JSON.parse(localStorage.getItem('selected_easybox') || 'null');
...
// =======================
// INIT HARTA MODAL CU DETECTIE JUDET DIN GEOLOCATION
// =======================
function initModalMap() {
  if (modalMapInitialized) {
    // daca e deja initializat, curatam marker-ele si permitem rebuild (pentru filtru nou)
    try {
    ...
    // NU returnam – continuam pentru rebuild
  }

  // incercam sa detectam judetul din geolocatie
  function buildMapWithCounty(county) {
    // golim vechii markers
    modalMarkers.forEach(m => { try { m.setMap(null); } catch(e){} });
    modalMarkers.length = 0;

    modalMap = createEasyBoxMap('pickup-map-modal', modalMarkers, county || '');
    if (!modalMap) return;
    ...
    setTimeout(() => {
      ...
      // daca exista marcaj salvat, centram pe el
      try {
        const saved = JSON.parse(localStorage.getItem('selected_easybox') || 'null');
        if (saved && saved.lat && saved.lng) {
          modalMap.setCenter({ lat: saved.lat, lng: saved.lng });
          modalMap.setZoom(14);
          return;
        }
      } catch(e){}
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      const county = await reverseGeocodeCounty(latlng);
      if (county) {
        buildMapWithCounty(county);
      } else {
        buildMapWithCounty('');
      }
      // marcheaza locatia user dar nu recentreaza harta
      if (modalMap) locateUserOnModalMap(); else { try { locateUserOnModalMap(); } catch(e){} }
    }, (err) => {
      // geoloc esuat, incercam saved county din localStorage
      try {
        const saved = JSON.parse(localStorage.getItem('selected_easybox') || 'null');
        const county = saved?.county || '';
        buildMapWithCounty(county);
      } catch(e){
        buildMapWithCounty('');
      }
    ...
    // fara geoloc, incercam saved sau nimic
    try {
      const saved = JSON.parse(localStorage.getItem('selected_easybox') || 'null');
      buildMapWithCounty(saved?.county || '');
    } catch(e){
      buildMapWithCounty('');
    }
  }
...
// ======================================
// LOCATIE UTILIZATOR: marcheaza, nu recentreaza
// ======================================
...
function selectEasyBox(box) {
    document.getElementById('shipping_method').value = 'easybox';
...
// small delay pentru a permite browser-ului să afișeze elementul
  setTimeout(() => {
    if (window.google && modalMap) {
      try {
        google.maps.event.trigger(modalMap, 'resize');

        // dacă ai markers, potrivește bounds manual
        if (modalMarkers && modalMarkers.length) {
          const b = new google.maps.LatLngBounds();
          modalMarkers.forEach(m => b.extend(m.getPosition()));
          modalMap.fitBounds(b);
          // păstrează zoom maxim
          const l = google.maps.event.addListenerOnce(modalMap, 'idle', () => {
            if (modalMap.getZoom() > 14) modalMap.setZoom(14);
            google.maps.event.removeListener(l);
          });
        }
      } catch(e) { console.warn('resize modalMap err', e); }
    }
  }, 120);
}, 160);
// =======================
...
// callback apelat de Google Maps (script tag are callback=initEasyBoxMap)
function initEasyBoxMap() {
  // API-ul Google Maps e gata. Nu fortam modalul; initializam harta de pe pagina daca e nevoie.
  try {
    if (typeof initPageMap === 'function') {
      // initPageMap va verifica if pickup-section vizibil si va crea pageMap
      initPageMap();
    }
  } catch (e) {
    console.warn('initEasyBoxMap: eroare la initPageMap', e);
  }
}
// =======================
// FUNCTII LIPSA (adauga-le)
// =======================

/**
 * Obtine locatie utilizator (callback cu obiect {lat,lng} sau null)
 * cb poate fi functie(userPos) sau function(err)
 */
function getUserLocation(cb) {
  if (typeof cb !== 'function') cb = function(){};
  if (!navigator.geolocation) {
    console.warn('Geolocation nu este suportat.');
    return cb(null);
  }
  navigator.geolocation.getCurrentPosition((pos) => {
    cb({ lat: pos.coords.latitude, lng: pos.coords.longitude });
  }, (err) => {
    console.warn('Eroare geoloc:', err && err.message);
    cb(null);
  }, { enableHighAccuracy: true, timeout: 7000, maximumAge: 0 });
}

/**
 * Aplica un obiect easybox catre formular / UI.
 * options: { closeModal: bool } - daca true inchide modal
 */
function applyBoxToForm(box, options = {}) {
  if (!box) return;
  // asigura input-urile exista
  const shipInput = document.getElementById('shipping_method');
  const pickupInput = document.getElementById('pickup_location');
  const cityInput = document.getElementById('easybox_city');
  const countyInput = document.getElementById('easybox_county');
  ...
  if (shipInput) shipInput.value = 'easybox';
  if (pickupInput) pickupInput.value = `${box.name} | ${box.address}`;
  if (cityInput) cityInput.value = box.city || '';
  if (countyInput) countyInput.value = box.county || '';

  // afiseaza sectiunea Ridicare personala
  const pickupSection = document.getElementById('pickup-section');
  if (pickupSection) pickupSection.style.display = 'block';
  ...
  if (details) {
    details.innerHTML = `
      ...
      <strong>${box.name}</strong>
      <p>${box.address}</p>
      <p>${box.city || ''}${box.city && box.county ? ', ' : ''}${box.county || ''}</p>
      <p>Cost livrare: 0,00 Lei</p>
    `;
  }

  // centru pe harta daca exista
  if (window.pageMap && box.lat && box.lng) {
    try {
      pageMap.setCenter({ lat: box.lat, lng: box.lng });
      pageMap.setZoom(15);
    } catch(e){}
  }
  if (window.modalMap && box.lat && box.lng) {
    try {
      modalMap.setCenter({ lat: box.lat, lng: box.lng });
      modalMap.setZoom(15);
    } catch(e){}
  ...
  // salveaza local (si eventual trimite la server prin AJAX daca doresti)
  try {
    localStorage.setItem('selected_easybox', JSON.stringify(box));
  } catch (e) { /* ignore */ }
...
/**
 * Initializeaza harta de pe pagina (pageMap) si marcheaza easybox-urile
 * id-ul containerului de pe pagina ar trebui sa fie 'pickup-map' (sau adapteaza)
 */
function initPageMap() {
  // daca deja initializat, curatam marcajele
  try {
    if (pageMapInitialized && pageMarkers && pageMarkers.length) {
      pageMarkers.forEach(m => { try { m.setMap(null); } catch(e){} });
      pageMarkers.length = 0;
    }
  } catch(e){}

  const mapEl = document.getElementById('pickup-map') || document.getElementById('pickup-map-page');
  if (!mapEl || !window.google || !google.maps) {
    // nu putem initializa acum - poate maps nu e incarcat sau elementul nu exista
    return;
  }

  pageMap = new google.maps.Map(mapEl, {
    center: { lat: 44.439, lng: 26.096 },
    zoom: 12,
    gestureHandling: 'greedy'
  });
  ...
  document.querySelectorAll('.easybox-item').forEach(div => {
    ...
    const box = {
      id: div.dataset.id || '',
      name: div.dataset.name || div.querySelector('strong')?.textContent || '',
      address: div.dataset.address || div.querySelector('span')?.textContent || '',
    ...
    marker.addListener('click', function() {
      applyBoxToForm(box, { closeModal: false });
    });

    // daca se da click pe elementul din lista, il asociem
    if (!div.dataset.boundToMap) {
      div.addEventListener('click', (e) => {
        e.stopPropagation();
        applyBoxToForm(box, { closeModal: true });
      });
      div.dataset.boundToMap = '1';
    }
  ...
  if (added === 0) {
    // incearca sa centram pe saved
    try {
      const saved = JSON.parse(localStorage.getItem('selected_easybox') || 'null');
      if (saved && saved.lat && saved.lng) {
        pageMap.setCenter({ lat: saved.lat, lng: saved.lng });
        pageMap.setZoom(13);
      }
    } catch(e){}
  } else if (added === 1) {
    pageMap.setCenter(bounds.getCenter());
    pageMap.setZoom(14);
  } else {
    pageMap.fitBounds(bounds);
    const listener = google.maps.event.addListenerOnce(pageMap, 'idle', () => {
      if (pageMap.getZoom() > 14) pageMap.setZoom(14);
      google.maps.event.removeListener(listener);
    });
  }
...
/**
 * Callback folosit cand selectezi pickup in card shipping:
 * primeste pozitia user (sau null) si afiseaza easybox-urile pe pagina
 */
function showEasyBoxPickup(userPos) {
  // Asigura sectiunea vizibila
  const pickupSection = document.getElementById('pickup-section');
  if (pickupSection) pickupSection.style.display = 'block';

  // initializeaza pageMap (maps trebuie sa fie gata)
  if (window.google && google.maps) {
    initPageMap();
    // marcheaza si poz utilizator
    if (userPos && pageMap) {
      if (userLocationMarker) try { userLocationMarker.setMap(null); } catch(e){}
      userLocationMarker = new google.maps.Marker({
        position: userPos,
        map: pageMap,
        title: 'Locatia ta',
      ...
      // nu recentram automat (optional)
    }
    try { google.maps.event.trigger(pageMap, 'resize'); } catch(e){}
  } else {
    // maps nu e gata - incercare ulterioara va crea harta prin callback initEasyBoxMap
    console.warn('Google Maps nu e inca disponibil pentru showEasyBoxPickup.');
  }
}
(function(){
  const listSel = document.querySelectorAll('.easybox-item');
  if(!listSel.length) return;

  function updateUIAfterSelect(box) {
    // hidden inputs
    const hidId = document.getElementById('easybox_id');
    const ship  = document.getElementById('shipping_method');
    const pickup = document.getElementById('pickup_location');
    const cityI = document.getElementById('easybox_city');
    const countyI = document.getElementById('easybox_county');

    if(hidId) hidId.value = box.id || '';
    if(ship) ship.value = 'easybox';
    if(pickup) pickup.value = (box.name ? box.name + ' | ' : '') + (box.address || '');
    if(cityI) cityI.value = box.city || '';
    if(countyI) countyI.value = box.county || '';

    // update shipping-details preview (daca exista)
    const details = document.getElementById('shipping-details');
    if(details){
      details.innerHTML = `
        <p>Ridicare personala din:</p>
    ...
    // inchide modalul (daca exista)
    const pm = document.getElementById('pickup-modal');
    if(pm) pm.style.display = 'none';
  }

  async function selectEasyboxAjax(easyboxId, el) {
    const fd = new FormData();
    fd.append('action','select_easybox');
    fd.append('easybox_id', String(easyboxId));
    ...
    try {
      ...
      if(!data.success){
        console.warn('Select easybox error:', data.error);
        alert('Nu s-a putut selecta punctul: ' + (data.error || 'eroare'));
        return;
      }

      // raspunsul contine obiectul box (id,name,address,city,county,lat,lng,...)
      const box = data.box;

      // actualizeaza hidden inputs si UI
      updateUIAfterSelect(box);

      // de asemenea populeaza localStorage (optional)
      try { localStorage.setItem('selected_easybox', JSON.stringify(box)); } catch(e){}

      // optional: marcheaza elementul selectat vizual
      document.querySelectorAll('.easybox-item').forEach(i => i.classList.remove('active-easybox'));
      if(el) el.classList.add('active-easybox');

    } catch (err) {
      console.error('fetch error selectEasyboxAjax', err);
  ...
  listSel.forEach(el => {
    ...
    el.addEventListener('click', (ev) => {
      ...
      if(!id){
        alert('Punct invalid (fara ID).');
        return;
      }

      // lectura rapida din DOM pentru fallback UI (daca server nu returneaza)
      const quickBox = {
        id: id,
        name: el.dataset.name || (el.querySelector('strong')?.textContent||''),
        address: el.dataset.address || (el.querySelector('span')?.textContent||''),
        city: el.dataset.city || '',
        county: el.dataset.county || ''
      };

      // apel AJAX + update UI la raspuns
      selectEasyboxAjax(id, el);
    });
  });
})();
// =======================
...
<script
  ...
  async
  defer>
.easybox-item.active-easybox { border-color:#0074D9; background:#f0f8ff; }
</script>
