...
/* =========================
   DETECTARE USER SAU GUEST
   nu se forțează login
========================= */
...
/* =========================
   PRELUARE ADRESE LIVRARE PENTRU USER
   si PRELUARE ADRESA GUEST DIN SESIUNE
========================= */
$addresses = [];
if ($user_id) {
    $stmt = $pdo->prepare("
...
// guest address preselectat (daca exista)
$guestAddress = null;
if (!$user_id && isset($_SESSION['guest_address_id']) && $_SESSION['guest_address_id']) {
    $gId = (int)$_SESSION['guest_address_id'];
    $stmt = $pdo->prepare("SELECT id, guest_id, name, email, phone, address, city, country FROM guest_addresses WHERE id = ? LIMIT 1");
    $stmt->execute([$gId]);
    $guestAddress = $stmt->fetch();
    // dacă vrei să folosești direct valorile pentru afișare sau pentru preselectare în formular
    if ($guestAddress) {
        $shipping_name    = $guestAddress['name'];
        $shipping_phone   = $guestAddress['phone'];
        $shipping_address = $guestAddress['address'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $easybox_city   = null;
    $easybox_county = null;
    $guest_address_id = null; // id-ul adresei guest, dacă e cazul

    try {
        // Dacă avem guest checkout și adresa guest trebuie salvată, validăm și salvăm înainte de tranzacție.
        if (!$user_id && $shipping_method !== 'easybox') {
            $g_name        = trim($_POST['guest_name'] ?? '');
            $g_email       = trim($_POST['guest_email'] ?? '');
            $g_phone       = trim($_POST['guest_phone'] ?? '');
            $g_country     = trim($_POST['guest_country'] ?? '');
            $g_city        = trim($_POST['guest_city'] ?? '');
            $g_addressline = trim($_POST['guest_address_line'] ?? '');

            if (!$g_name || !$g_phone || !$g_city || !$g_addressline) {
                throw new Exception("Completează nume, telefon și adresă");
            }

            // Inserăm adresa guest în afara tranzacției principale, ca să nu se șteargă la rollback.
            // folosim guest_id = 0 ca să nu avem constraint NOT NULL
            $stmtGuest = $pdo->prepare("
                INSERT INTO guest_addresses
                (guest_id, name, email, phone, address, city, country, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
            $stmtGuest->execute([0, $g_name, $g_email, $g_phone, $g_addressline, $g_city, $g_country]);
            $guest_address_id = $pdo->lastInsertId();

            // salvez id-ul in sesiune pentru a ramane selectata/afisata pe pagina ContinuaComanda
            $_SESSION['guest_address_id'] = $guest_address_id;

            // Pregătim datele de livrare pentru folosire mai jos
            $shipping_name    = $g_name;
            $shipping_phone   = $g_phone;
            $shipping_address = ($g_country ? $g_country . ', ' : '') . "{$g_city}, {$g_addressline}";
        }

        // Pentru easybox cu guest, verificăm telefonul și pregătim datele. Inserarea guest poate fi făcută tot înainte
        if (!$user_id && $shipping_method === 'easybox') {
            $g_phone = trim($_POST['guest_phone'] ?? '');
            if (!$g_phone) {
                throw new Exception("Telefon pentru ridicare EasyBox lipsă");
            }
            // nu este obligatoriu să inserăm guest_addresses pentru easybox; dacă vrei asta, poți insera similar cu de mai sus
        }

        // Acum deschidem tranzacția pentru operațiunile comenzii.
        ...
        if ($shipping_method === 'easybox') {
            ...
            $shipping_phone   = $user['phone'] ?? trim($_POST['guest_phone'] ?? '');
            if (!$shipping_phone) {
                throw new Exception("Telefon pentru ridicare EasyBox lipsă");
            }
            ...
            if ($user_id) {
                ...
                // dacă am ajuns aici înseamnă că am salvat deja datele guest mai sus
                // $shipping_name, $shipping_phone, $shipping_address sunt deja setate
                // asigurăm că guest_address_id rămâne în sesiune
                if (isset($guest_address_id) && $guest_address_id) {
                    $_SESSION['guest_address_id'] = $guest_address_id;
                }
            }
        }
        ...
        /* =========================
           INSERARE COMANDA
           user_id poate fi NULL pentru guest
           dacă tabela orders are coloana guest_address_id o completăm
        ========================== */

        // verificăm dacă tabelul orders are coloana guest_address_id
        $hasGuestCol = false;
        $colCheck = $pdo->query("SHOW COLUMNS FROM orders LIKE 'guest_address_id'")->fetch();
        if ($colCheck) $hasGuestCol = true;
        ...
        if ($hasGuestCol) {
            ...
            // fallback, inserăm fără guest_address_id
            $stmt = $pdo->prepare("
                INSERT INTO orders
                (
                    user_id, total, shipping_method,
        ...
        // dacă am salvat guest_adress în afara tranzacției, acesta rămâne
        $pdo->rollBack();
...
<div id="address-modal" class="address-modal">
    ...
    <form id="addressForm" novalidate>
        ...
        <input type="text" name="contact_name" required minlength="3" maxlength="80"
               pattern="^[A-Za-zĂÂÎȘȚăâîșț .'-]{3,80}$"
        ...
        <input type="text" name="city" required minlength="2" maxlength="80"
               pattern="^[A-Za-zĂÂÎȘȚăâîșț .'-]{2,80}$"
               placeholder="Ex. Craiova">
    
        <label>Adresă</label>
        <textarea name="address_line" required minlength="5" maxlength="120"
                  placeholder="Stradă, număr, bloc, scară, apartament"></textarea>
...
(function(){
    ...
    if (addressForm && !addressForm.dataset.bound) {
        ...
        return; // oprește complet execuția
        }
            ...
            const county = (this.querySelector('[name="county"]')?.value || '').trim();
            const city = (this.querySelector('input[name="city"]')?.value || '').trim();
            const address_line = (this.querySelector('[name="address_line"]')?.value || '').trim();
...
function validateAddressForm(form) {
    ...
    const address = form.querySelector('[name="address_line"]');
    ...
    const nameRegex = /^[A-Za-zĂÂÎȘȚăâîșț .'-]{3,80}$/u;
    const cityRegex = /^[A-Za-zĂÂÎȘȚăâîșț .'-]{2,80}$/u;
    const invalidCities = ['localitate', 'oras', 'city', 'test'];
    if (invalidCities.includes(cityValue.toLowerCase())) {
        alert('Introdu o localitate reala.');
        city.focus();
    return false;
    }

    if (!nameValue || !phoneValue || !countyValue || !cityValue || !addressValue) {
        alert('Completeaza toate campurile.');
        return false;
    }

    if (!allowedCounties.includes(countyValue)) {
        alert('Judetul nu este valid.');
        county.focus();
        return false;
    }
    ...
    if (!/^07[0-9]{8}$/.test(phoneValue)) {
        alert('Telefonul trebuie sa fie valid, de forma 07xxxxxxxx.');
        phone.focus();
        return false;
    }

    if (!cityRegex.test(cityValue)) {
        alert('Localitatea nu este valida.');
        city.focus();
        return false;
    }

    // ✔️ VALIDARE CORECTA ADRESA
    const normalizedAddress = addressValue.replace(/\s+/g, ' ').trim().toLowerCase();
    const invalidAddressWords = ['adresa', 'test', 'asdf', 'qwerty'];
    const addressRegex = /^(str\.?|strada|bd\.?|bulevardul|aleea|intrarea|calea)\s+[a-zăâîșț0-9 .'-]+\s+\d+/i;
    const containsInvalidWord = invalidAddressWords.some(word => normalizedAddress.includes(word));

    if (!addressRegex.test(normalizedAddress) || containsInvalidWord) {
        alert('Adresa invalida. Ex: Strada Mihai Viteazu 12');
        address.focus();
        return false;
    }
    
    address.value = addressValue.replace(/\s+/g, ' ').trim();

    return true;
}
document.getElementById('addressForm').addEventListener('submit', function(e) {
    if (!validateAddressForm(this)) {
        e.preventDefault(); // oprește submit-ul dacă nu e valid
    }
});
</script>
