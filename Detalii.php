...
if ($imgNum !== null) {
    ...
    } elseif ($imgNum >= 81 && $imgNum <= 124) {
        $componentsText = 'Tablourile sunt realizate pe foaie A3, echivalent a doua coli A4, conform standardului ISO 216';
    } elseif ($imgNum >= 131 && $imgNum <= 147) {
        $componentsText = 'Covoare rotunde PsyGeometry, mocheta imprimata, spate cauciuc antiderapant';
    }
}
...
if ($isRound && array_key_exists('diameter_m', $item) && $item['diameter_m'] !== null && $item['diameter_m'] !== '') {
    $dimensionsText = 'Diametru ' . formatMeters($item['diameter_m']);
} else {
    // logica standard pentru lungime/latime, neschimbată
